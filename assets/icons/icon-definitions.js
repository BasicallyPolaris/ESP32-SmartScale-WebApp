...
const iconDefinitions = {
  ...
  "scale-balanced": `<circle cx="12" cy="5" r="3"/><path d="M6.5 8a2 2 0 0 0-1.905 1.46L2.1 18.5A2 2 0 0 0 4 21h16a2 2 0 0 0 1.925-2.54L19.4 9.5A2 2 0 0 0 17.48 8Z"/>`,

  "balance-scale": `<path d="m16 16 3-8 3 8c-.87.65-1.92 1-3 1s-2.13-.35-3-1Z"/><path d="m2 16 3-8 3 8c-.87.65-1.92 1-3 1s-2.13-.35-3-1Z"/><path d="M7 21h10"/><path d="M12 3v18"/><path d="M3 7h2c2 0 5-1 7-2 2 1 5 2 7 2h2"/>`,
  ...
  "cookie-bite": `<path d="M12 2a10 10 0 1 0 10 10 4 4 0 0 1-5-5 4 4 0 0 1-5-5"/><path d="M8.5 8.5v.01"/><path d="M16 15.5v.01"/><path d="M12 12v.01"/><path d="M11 17v.01"/><path d="M7 14v.01"/>`,
...
// Function to create an inline SVG icon
function createIcon(iconName, className = "icon") {
  const iconPath = iconDefinitions[iconName];
  if (!iconPath) {
    console.warn(`Icon "${iconName}" not found`);
    return `<div class="${className}">?</div>`;
  }

  return `<svg class="${className}" viewBox="0 0 24 24">${iconPath}</svg>`;
}

// Function to replace i tags and div icons with inline SVGs
function replaceIconTags() {
  const iconElements = document.querySelectorAll(
    'i[class*="icon-"], div[class*="icon-"]'
  );
  ...
  iconElements.forEach((element) => {
    ...
    if (iconClass) {
      ...
      const otherClasses = classes
        ...
        .join(" ");
      const finalClass = otherClasses ? `icon ${otherClasses}` : "icon";

      const svgElement = document.createElement("div");
      svgElement.innerHTML = createIcon(iconName, finalClass);

      // Replace the element with the SVG
      element.parentNode.replaceChild(svgElement.firstChild, element);
    }
  });
}
